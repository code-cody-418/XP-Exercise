...
export const VideoPlayer = ({ profile }) => {
  ...
  const expUp = () => {
    if (profile === null) {
      ...
      httpConfig
        .put(`/apis/profile/expUp/${profile.profileId}`, profile)
        .then((reply) => {
          if (reply.status === 200) {
            // console.log(reply);
  ...
  const levelUp = () => {
    if (profile === null) {
      ...
      if ((profile.profileExp + "").indexOf("00", 1) > -1 === true) {
        httpConfig
          .put(`/apis/profile/levelUp/${profile.profileId}`, profile)
          .then((reply) => {
            if (reply.status === 200) {
              // console.log(reply);
              dispatch(fetchProfileByProfileId(profile.profileId));
  ...
  const coinUp = () => {
    if (profile === null) {
      ...
      httpConfig
        .put(`/apis/profile/coinUp/${profile.profileId}`, profile)
        .then((reply) => {
          if (reply.status === 200) {
            // console.log(reply);
            dispatch(fetchProfileByProfileId(profile.profileId));
  ...
  const [playingThirtySecondAudio, setPlayingThirtySecondAudio] =
    useState(false);
